...
def test_mask_strategy():
    ...
    assert strategy.apply(input_data) == expected_output, "mask strategy test failed"
...
def test_hash_strategy():
    ...
    assert strategy.apply(input_data) == expected_output, "hash strategy test failed"    

def test_block_strategy():
    strategy = BlockStrategy()
    input_data = [("My", "O"), ("email", "O"), ("is", "O"), ("johndoe@email.com", "B-EMAIL")]
    expected_output = "<BLOCKED>"
    assert strategy.apply(input_data) == expected_output, "blocking strategy test failed"

def test_partial_masking_strategy():
    strategy = PartialMasking()
    input_data = [("My", "O"), ("email", "O"), ("is", "O"), ("johndoe@email.com", "B-EMAIL")]
    expected_output = "My email is ***om"
    assert strategy.apply(input_data) == expected_output, "partial masking strategy test failed"
